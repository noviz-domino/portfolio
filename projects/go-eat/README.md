...
**목록** — 사이드바에 "24곳 중 17곳 정복" 진행률이 뜬다. 필터를 걸어도 이 수치는 전체를 기준으로 센다.

![목록 화면](docs/screenshots/01-목록.png)

| 상세 | 등록 폼 | 모바일 |
|:--:|:--:|:--:|
| ![](docs/screenshots/02-상세.png) | ![](docs/screenshots/03-등록폼.png) | ![](docs/screenshots/04-모바일.png) |

등록 폼 아래에 *"저장하면 목록 카드용 한 줄 요약을 AI가 자동으로 만들어줍니다"* 라고 적어두었다. 메모를 길게 써도 목록에서는 한 줄로 보이게 하기 위한 기능이다.

## 데모 계정

이 앱은 로그인해야 쓸 수 있다. 직접 눌러보려면 아래 계정을 쓰면 된다.
